        ...
        public async Task<ChatMessage> SaveChatMessageAsync(ChatMessage message)
{
    try
    {
        await _conn.OpenAsync();
        using var cmd = new NpgsqlCommand(
            @"INSERT INTO t_chat_messages (c_sender_id, c_recipient_id, c_content, c_timestamp, c_is_read)
              VALUES (@senderId, @recipientId, @content, @timestamp, @isRead)
              RETURNING *",
            _conn);
        
        cmd.Parameters.AddWithValue("@senderId", message.c_sender_id);
        cmd.Parameters.AddWithValue("@recipientId", message.c_recipient_id);
        cmd.Parameters.AddWithValue("@content", message.c_content);
        cmd.Parameters.AddWithValue("@timestamp", message.c_timestamp);
        cmd.Parameters.AddWithValue("@isRead", message.c_is_read);
        ...
        if (await reader.ReadAsync())
        {
            ...
            {
                c_message_id = reader.GetInt32(reader.GetOrdinal("c_message_id")),
                c_sender_id = reader.GetInt32(reader.GetOrdinal("c_sender_id")),
                c_recipient_id = reader.GetInt32(reader.GetOrdinal("c_recipient_id")),
                c_content = reader.GetString(reader.GetOrdinal("c_content")),
                c_timestamp = reader.GetDateTime(reader.GetOrdinal("c_timestamp")),
                c_is_read = reader.GetBoolean(reader.GetOrdinal("c_is_read"))
            };
        }
        return null;
    }
    ...
    try
    {
        ...
        using var cmd = new NpgsqlCommand(
            @"SELECT * FROM t_chat_messages 
              WHERE (c_sender_id = @senderId AND c_recipient_id = @recipientId) 
                 OR (c_sender_id = @recipientId AND c_recipient_id = @senderId)
